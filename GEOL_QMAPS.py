...
class GEOL_QMAPS:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&GEOL_QMAPS')
        # TODO: We are going to let the user set this up in a future iteration
        self.toolbar = self.iface.addToolBar(u'GEOL_QMAPS')
        self.toolbar.setObjectName(u'GEOL_QMAPS')
    ...
    def fill_Table2(self, fichier_output):
        ...
        list_lithology_reference = self.get_first_column_text(self.dictionaries_path, "General // List of all lithologies")
        """emplacement_99_CSV_files = self.templateCSV_path

        # Reference list of lithologies
        if os.path.exists(emplacement_99_CSV_files):
            fichier_reference = pd.read_csv(
                emplacement_99_CSV_files + "Lithologies.csv", sep=";"
            )
            list_lithology_reference = list(fichier_reference["Valeur"])

        else:
            self.iface.messageBar().pushMessage(
                "Layer not found: " + emplacement_99_CSV_files,
                level=Qgis.Warning,
                duration=45,
            )"""
    ...
    def button_edit2(self, row):
        ...
        litho_local = self.get_first_column_text(self.dictionaries_path, "Local lithologies/List of lithologies")
        litho_supergene = self.get_first_column_text(self.dictionaries_path, "Supergene lithologies/List of lithologies")
        litho_sedimentary = self.get_first_column_text(self.dictionaries_path, "Sedimentary lithologies/List of lithologies")
        litho_volcanoclastic = self.get_first_column_text(self.dictionaries_path, "Volcanoclastic lithologies/List of lithologies (clast size-based)")
        litho_igneous_extrusive = self.get_first_column_text(self.dictionaries_path, "Igneous extrusive lithologies/List of lithologies")
        litho_igneous_intrusive = self.get_first_column_text(self.dictionaries_path, "Igneous intrusive lithologies/List of lithologies")
        litho_metamorphic = self.get_first_column_text(self.dictionaries_path, "Metamorphic lithologies/List of lithologies")
    ...
    def lithologies_sorting(
        ...
        for k in range(0, len(fichier_output["Litho"])):
            ...
            if r in litho_local and r != "Unknown":
                fichier_output_lithology=self.add_row_to_fichier_output_lithology(k,fichier_output_lithology,fichier_output,"Local lithologies_PT_",list_columns_check3,filename_without_extension)

            # litho_supergene :
            elif r in litho_supergene and r != "Unknown":
                fichier_output_lithology=self.add_row_to_fichier_output_lithology(k,fichier_output_lithology,fichier_output,"Supergene lithologies_PT_",list_columns_check3,filename_without_extension)

            # litho_sedimentary :
            elif r in litho_sedimentary and r != "Unknown":
                fichier_output_lithology=self.add_row_to_fichier_output_lithology(k,fichier_output_lithology,fichier_output,"Sedimentary lithologies_PT_",list_columns_check3,filename_without_extension)

            # litho_volcanoclastic :
            elif r in litho_volcanoclastic and r != "Unknown":
                fichier_output_lithology=self.add_row_to_fichier_output_lithology(k,fichier_output_lithology,fichier_output,"Volcanoclastic lithologies_PT_",list_columns_check3,filename_without_extension)
            
            # litho_volcanic :
            elif r in litho_igneous_extrusive and r != "Unknown":
                fichier_output_lithology=self.add_row_to_fichier_output_lithology(k,fichier_output_lithology,fichier_output,"Igneous extrusive lithologies_PT_",list_columns_check3,filename_without_extension)

            # litho_plutonic :
            elif r in litho_igneous_intrusive and r != "Unknown":
                fichier_output_lithology=self.add_row_to_fichier_output_lithology(k,fichier_output_lithology,fichier_output,"Igneous intrusive lithologies_PT_",list_columns_check3,filename_without_extension)

            # litho_metamorphic :
            elif r in litho_metamorphic and r != "Unknown":
                fichier_output_lithology=self.add_row_to_fichier_output_lithology(k,fichier_output_lithology,fichier_output,"Metamorphic lithologies_PT_",list_columns_check3,filename_without_extension)

        return fichier_output_lithology
    
    #add each row to the master dictioanry of pandas
    def add_row_to_fichier_output_lithology(self,k,fichier_output_lithology,fichier_output,name_prefix,list_columns_check3,filename_without_extension):
            ...
            if type_structure2 != "NULL":
                ...
                for sheet in worksheets_structure:
                    ...
                    if type_structure2 == sheet+suffix:
                        ...
                        header_reference = list(fichier_output_structures[sheet+suffix].columns)
                        ...
                        fichier_output_structures[sheet+suffix].loc[len(fichier_output_structures[sheet+suffix])]=ligne
    ...
    ### CSV TOOLS ###
    def delete_row_from_csv_layer(self,gpkg_path, layer_name, column_name, value_to_delete):
        """
        ...
        try:
            ...
            updated_features = [
                feature for feature in features if feature['properties'][column_name] != value_to_delete
            ]
        ...
        try:
            ...
            for key in new_row.keys():
                if key not in layer_schema['properties']:
                    raise ValueError(f"Invalid column name '{key}'. Column does not exist in the layer.")
    ...
    def addCsvItem(self):
        ...
        if "List of lithologies" in current_layer:
            self.add_row_to_csv_layer(self.dictionaries_path, "General // List of all lithologies", new_row)
            
            # Updates the layer in QGIS
        layer_csv = QgsProject.instance().mapLayersByName(current_layer)[0]
    ...
    def deleteCsvItem(self):
        ...
        if "List of lithologies" in current_layer :
            ...
            self.delete_row_from_csv_layer(self.dictionaries_path, "General // List of all lithologies", "Valeur", delete_item)
    ...
    def set_user_by_default(self):
        ...
        if self.dlg.lineEdit_39.text():
            ...
            liste_users=self.get_first_column_text(self.dictionaries_path, "General // List of Users")
            ...
            if 100 not in list_score:
                ...
                self.add_row_to_csv_layer(self.dictionaries_path, "General // List of Users", new_row)


                """user_file = pd.concat(
                    [user_file, pd.DataFrame([new_row])], ignore_index=True
                )
                user_file.sort_values(by="Valeur", inplace=True)

                # Rewrites the CSV file with the added line
                user_file.to_csv(
                    emplacement_User_file, encoding="latin_1", sep=";", index=False
                )"""

                # Updates the layer in QGIS
                layer_user = QgsProject.instance().mapLayersByName("General // List of Users")[0]
    ...
    def removeDuplicates(self):
        ...
        for layer in project.mapLayers().values():
            # Check if the layer name matches the target name
            # if layer.name() in shps.index.tolist():
            if layer.name().startswith("Compilation_") or layer.name() in shps.index.tolist() :
    ...
    def get_first_column_text(self,gpkg_path, layer_name):
        ...
        try:
            ...
            with fiona.open(gpkg_path, layer=layer_name) as layer:
                ...
                first_text_column = next((field for field, field_type in fields.items() if field_type.startswith('str')), None)
                ...
                text_values = [feature['properties'][first_text_column] for feature in layer if feature['properties'][first_text_column] is not None]
    ...
    def merge_text_tables_remove_duplicates(self,gpkg1_path, layer1_name, gpkg2_path, layer2_name, output_gpkg_path, output_layer_name):
        """
    ...
    def merge_current_to_existing(self):
        ...
        for layer in project.mapLayers().values():
            ...
            if (layer.name() in shps.index.tolist() ):
                ...
                compilation_layer_path = compilationCompGpkgPath + "|layername=Compilation_" + layer.name()

                # Load the layer into memory
                layer_temp = QgsVectorLayer(f"{current_layer_path}", "temp_layer", "ogr")
                ...
                if layer_temp.isValid():
                    ...
                    print(f"Error: Could not load layer '{layer_temp}' from '{current_layer_path}'.")

                if(feature_count>0):
                    params = {
                        "LAYERS": [current_layer_path, compilation_layer_path],
                        "OUTPUT": "ogr:dbname='"
                        + self.mynormpath(compilationCompGpkgPath + "' table=\"" + "Compilation_" +layer.name())
                        + '"',
                    }
    ...
    def mergeProjects(self):
        ...
        if (
            ...
            for file in csvs:
                ...
                self.merge_text_tables_remove_duplicates(main_path, file, sub_path, file, merge_path, file)
            ...
            src_file = main_project_path + self.dir_0 + "/CURRENT_MISSION+DICTIONARIES.qlr"
            dst_file = (
            ...
            for layer in project.mapLayers().values():
                # Check if the layer name matches the target name
                if (layer.name().replace("Compilation_","") in shps.index.tolist() and layer.name().startswith("Compilation_") ):
                    ...
                    params = {
                        "LAYERS": [main_layer_path, sub_layer_path],
                        "OUTPUT": "ogr:dbname='"
                        + self.mynormpath(mergeCompGpkgPath + "' table=\"" + layer.name())
                        + '"',
                    }
                    merged_layers = processing.run("native:mergevectorlayers", params)
    ...
    def exportData(self):
        ...
        if os.path.exists(self.mynormpath(self.dlg.lineEdit_7.text())):
            ...
            layers_names = [
                ...
                "Compilation_Deformation_zones_PG",
            ...
            file.append(
                self.geopackage_file_path + "|layername=Compilation_Deformation_zones_PG"
            )
            """file.append(
                self.geopackage_file_path
    ...
    def virtualStops(self):
        ...
        if self.dlg.lineEdit_53.text():
            ...
            file.append(
                self.geopackage_file_path + "|layername=Compilation_Folds_PT"
            )
    ...
    def update_combobox_delete(self):
        ...
        list_combobox_delete=self.get_first_column_text(self.dictionaries_path, self.dlg.comboBox.currentText())
    ...
    def check_version(self):
        ...
        template_version_file_path = self.mynormpath(self.basePath + self.dir_99 + "Version.txt")
    ...
    def use_exif_azimuth(self):
        ...
        if not layer.isValid():
            ...
            for feature in layer.getFeatures():
                ...
                manipulated_value = feature["Azimuth"] + dec
                
                # Commit the changes to the feature
                if not layer.dataProvider().changeAttributeValues({feature_id: {"Azimuth": manipulated_value}}):
                    print(f"Failed to update Azimuth for feature ID {feature_id}")
            

        if(dec != ""):

            self.iface.messageBar().pushMessage(
                "Photo orientation updated to account for declination {}".format(round(dec,2)),
                level=Qgis.Success,
                duration=15,
            )   
